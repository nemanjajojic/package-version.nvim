...
M.run_async = function(package_config)
	if not mutex.try_lock("Composer Delete") then
		return
	end
	...
	window.display_select("Select Dependency Type", options, function(dep_type)
		...
		local on_exit = function(job_id, code, event)
			...
			if code ~= 0 then
				local cmd_display = "composer remove " .. package_name
				if dep_type ~= "" then
		...
		local cmd = "composer remove " .. package_name .. " --no-ansi --no-interaction --no-audit"
		if dep_type ~= "" then
			cmd = cmd .. " " .. dep_type
		end
		local update_one_command = common.prepare_composer_command(cmd, docker_config, true)
		...
		timeout_timer = common.start_job_timeout(job_id, timeout_seconds, "Composer delete command", function()
			mutex.unlock()
			spinner.hide()
		end)
	end)
