...
---@param packages table<string, {version: string, latest: string, status: string, abandoned: boolean}> Package data
---@param namespace_id number
---@param color_config table
---@param latest_hl string
---@param wanted_hl string
---@param abandoned_hl string
local function display_packages(packages, namespace_id, color_config, latest_hl, wanted_hl, abandoned_hl)
...
M.run_async = function(package_config)
	if not mutex.try_lock("Composer Outdated") then
		return
	end
	...
	if cache.is_enabled(cache_config) then
		local cached_packages = cache.get(cache.PACKAGE_MANAGER.COMPOSER, cache.OPERATION.OUTDATED)
		if cached_packages then
			display_packages(cached_packages, namespace_id, color_config, latest_hl, wanted_hl, abandoned_hl)
	...
	local on_exit = function(job_id, code, event)
		...
		if cache.is_enabled(cache_config) then
			...
			cache.set(cache.PACKAGE_MANAGER.COMPOSER, cache.OPERATION.OUTDATED, packages, ttl)
		end

		display_packages(packages, namespace_id, color_config, latest_hl, wanted_hl, abandoned_hl)
	...
	local outdated_command = common.prepare_composer_command("composer outdated --direct --format=json", docker_config, true)
...
M.warmup_cache = function(package_config)
	...
	vim.fn.jobstart(outdated_command, {
		...
		on_exit = function(_, code, _)
			...
			cache.set(cache.PACKAGE_MANAGER.COMPOSER, cache.OPERATION.OUTDATED, packages, warmup_ttl)
		end,
	})
end
