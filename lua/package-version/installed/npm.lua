...
M.run_async = function(package_config)
	if not mutex.try_lock("NPM Installed") then
		return
	end
	...
	if cache.is_enabled(cache_config) then
		local cached_packages = cache.get(cache.PACKAGE_MANAGER.NPM, cache.OPERATION.INSTALLED)
	...
	local installed_command = common.prepare_npm_command("npm list --depth=0 --package-lock-only --json", docker_config)
	...
	timeout_timer = common.start_job_timeout(job_id, timeout_seconds, "NPM installed command", function()
		mutex.unlock()
