	...
	window.display_select("Select Update Scope", options, function(update_scope)
		...
		local on_exit = function(job_id, code, event)
			...
			if code ~= 0 then
				...
				window.display_error(stderr_lines, "composer update " .. package_name)
			...
			window.display_success(stderr_lines, "composer update " .. package_name)

			mutex.unlock()
		end

		local docker_config = common.get_docker_config(package_config)
		local cmd = "composer update " .. package_name .. " --no-audit --no-ansi"
