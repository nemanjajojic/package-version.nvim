...
M.run_async = function(package_config)
	if not mutex.try_lock("Yarn Outdated") then
		return
	end
	...
	if cache.is_enabled(cache_config) then
		local cached_packages = cache.get(cache.PACKAGE_MANAGER.YARN, cache.OPERATION.OUTDATED)
	...
	local on_exit = function(job_id, code, event)
		...
		display_packages(packages, namespace_id, color_config, latest_hl, wanted_hl)
	...
	local outdated_command = common.prepare_yarn_command("yarn outdated --json", docker_config)
...
M.warmup_cache = function(package_config)
	...
	-- Skip if valid cache already exists (handles missing + expired)
	local cached_packages = cache.get(cache.PACKAGE_MANAGER.YARN, cache.OPERATION.OUTDATED)
	if cached_packages then
		return
	end
