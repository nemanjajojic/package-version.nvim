...
---@param packages table<string, {current: string, wanted: string, latest: string, isDeprecated: boolean}> Package data
...
M.run_async = function(package_config)
	if not mutex.try_lock("PNPM Outdated") then
		return
	end
	...
	if cache.is_enabled(cache_config) then
		local cached_packages = cache.get(cache.PACKAGE_MANAGER.PNPM, cache.OPERATION.OUTDATED)
	...
	local on_exit = function(job_id, code, event)
		...
		if cache.is_enabled(cache_config) then
			...
			cache.set(cache.PACKAGE_MANAGER.PNPM, cache.OPERATION.OUTDATED, result, ttl)
		end

		display_packages(result, namespace_id, color_config, latest_hl, wanted_hl, abandoned_hl)
	...
	local outdated_command = common.prepare_pnpm_command("pnpm outdated --json", docker_config)
...
M.warmup_cache = function(package_config)
	...
	vim.fn.jobstart(outdated_command, {
		...
		on_exit = function(_, code, _)
			...
			cache.set(cache.PACKAGE_MANAGER.PNPM, cache.OPERATION.OUTDATED, result, warmup_ttl)
		end,
	})
end
