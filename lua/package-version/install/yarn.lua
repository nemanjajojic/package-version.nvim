...
M.run_async = function(package_config)
	...
	local on_exit = function(job_id, code, event)
		...
		if code ~= 0 then
			...
			window.display_error(stderr_lines, "yarn install")

			return
		end

		-- Display success output (yarn writes to stderr)
		window.display_success(stderr_lines, "yarn install")
	...
	local install_command = common.prepare_yarn_command("yarn install --silent", docker_config)
